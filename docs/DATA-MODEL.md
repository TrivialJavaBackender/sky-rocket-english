...
1. **Контент отделён от прогресса.** Контент-таблицы заполняются sync-скриптом из `content/<course>/` идемпотентно по `content_hash` (паттерн проверен в interview-prep). Прогресс — только пользовательские таблицы; контент можно пересинкать без потери прогресса.
