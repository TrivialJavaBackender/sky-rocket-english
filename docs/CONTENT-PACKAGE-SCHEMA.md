...
Родовая схема, одинаковая для всех курсов. Каждый модуль — самодостаточный пакет в `courses/<slug>/content/module-{NN}/`. Пакеты синкаются в Neon идемпотентно по `content_hash`. Правило языка: **целевое ядро — на целевом языке** (термины, примеры, тексты, рамки заданий, ответы, модельное письмо), graded под уровень. Объяснительные леса (`explanation`, `definition`, глоссы, `note`, `checklist`, `goals`) — на целевом при погружении или на **метаязыке** курса, если он объявлен (`docs/COURSE-DESIGN-GUIDE.md` §10; граница по полям — в `PLAN.md` §5 курса). Движок язык лесов не валидирует.
...
#### Правило чипа (key_word_transformation)

Плеер печатает `key` чипом над пропуском, и **регистр чипа объявляет, что он значит** — тот же сигнал, который читает учащийся:

| Форма | Смысл чипа | Когда |
|---|---|---|
| `ЗАГЛАВНЫМИ` — «ключевое слово» | «перепиши так, чтобы **это слово** встало в пропуск без изменений» (формат CAE Use of English Part 4) | трансформация = вставка заданного слова. en-c1 — весь такой |
| `строчными` — «инструкция» | называет **правило**, которое отрабатывается: `wir-form`, `polite form (Sie)`, `Perfekt with sein`, `adjective ending after den` | трансформация = смена формы, которую нельзя выразить одним готовым словом. Пропуск при этом остаётся коротким (1–2 слова) |

`KeyWordTransformationContentSchema` (`web/lib/content-schema.ts`) проверяет каждый режим своим набором правил; sync и `pnpm validate-content` валятся на нарушении.

**Чип-ключевое слово (ЗАГЛАВНЫМИ):** входит в **каждый** элемент `answers` как последовательность слов; ответ **длиннее** ключа; ключ **не встречается** в `pre`/`post`. Заглавная метка операции (`FRAGE`, `PERFEKT`) — брак: она обещает слово, которого в ответе нет. `key: NEUEN` при `answers: [neuen]` — тоже брак: чип показывает ответ целиком. `key: WIR` при `pre: "Wir "` — брак: чип требует слово, которого не хватает.

**Чип-инструкция (строчными):** не должен встречаться внутри `answers` — иначе метка выдаёт формулировку ответа. Пишется на **метаязыке курса** (`PLAN.md` §5): в de-a1/de-a2 — по-английски, грамматические термины и словарные формы целевого языка внутри метки допустимы (`nehmen (ich-form)`, `kein in the accusative`, `Präteritum of sein`).

В обоих режимах `answer_shown` ∈ `answers` — иначе показанный после проверки образец не прошёл бы собственный грейдинг. Сравнение регистро- и пунктуационно-нечувствительное, `ß` сворачивается в `ss` (чип `HEISSEN` матчится с ответом `heißen`).

**Пропуск не расширяют ради чипа.** Если под ЗАГЛАВНОЕ ключевое слово ответ приходится растягивать до 4–6 слов, это признак, что трансформация здесь — смена формы: нужен строчный чип-инструкция и короткий пропуск.

```yaml
  - type: key_word_transformation      # чип-инструкция: рамка уже содержит Wir,
    group: grammar                     # чип называет правило, ответ — одно слово
    content: {s1: Ich habe zwei Brüder., key: wir-form, pre: "Wir ", post: " zwei Brüder.",
              answers: [haben], hint: 1 Wort, answer_shown: haben}
``` Если сформулировать задание без метки-операции не выходит, значит для этой цели нужен другой тип упражнения (`grammar_drill`, `error_correction`), а не `key_word_transformation`.
