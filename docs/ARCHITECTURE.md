...
3. **import** — копирует новые блобы из кеша tts-mcp в `web/public/audio`, пишет коммитируемый `courses/<slug>/audio/manifest.json` (отсортирован по `text_hash` — стабильный diff), прунит из `web/public/audio/<lang>/**` файлы, не упомянутые ни одним манифестом этого языка, печатает отчёт (клипов на модуль, суммарный вес, непросинтезированные фразы, фразы длиннее `MAX_SENTENCE_CHARS`).
...
Гранулярность клипа для текстов — одна константа `READING_CLIP_GRANULARITY` в `lib/domain/audio-text.ts`, которую читают обе стороны пайплайна: генератор (`scripts/audio.ts`) и резолвер (`paragraphClipTexts` в `lib/use-cases/audio.ts`). Сейчас **клип = абзац**: просодия идёт сквозь границы предложений, а в очереди `playSequence` нет швов между ними. Плата за это — риск со стороны движка: Chatterbox рендерит вход одним авторегрессивным проходом без внутреннего разбиения (в отличие от Piper с его `join_sentences`), поэтому длинный абзац — самая вероятная форма для обрыва или галлюцинации, и один плохой дубль стоит целого абзаца, а не предложения; отчёт `pnpm audio` отдельно перечисляет фразы длиннее `MAX_SENTENCE_CHARS` именно как кандидатов на ручное прослушивание. Значение `'sentence'` — консервативный откат (мелкие рендеры, дешёвые пересъёмки), интерфейс от переключения не меняется вообще: `ReadingText` в обоих случаях получает массив клипов на абзац. Флип константы меняет текст каждого клипа текстов, а значит и его хеш, — за ним обязаны идти `pnpm audio` и `pnpm sync`, иначе ▶ у текстов просто исчезнут (тот же fail-safe D15).
